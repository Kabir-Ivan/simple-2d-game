...
var player = {
  ...
  object_type: "unit",
  pos: [0, 0],
  sprite: new Sprite("img/player_2.png", [0, 0], [124, 100], 16, [0], 180),
};
var terrain = [
  {
    pos: [-800, -600],
...
var freq = [1000, 2000];
var left = Math.round(freq[0] + Math.random() * (freq[1] - freq[0]));
for (var i = -100; i < map_height / 20; i++) {
  for (var j = -100; j < map_width / 20; j++) {
      left--;
      if (left <= 0) {
        stars.push({
          object_type: "star",
        ...
        left = Math.round(freq[0] + Math.random() * (freq[1] - freq[0]));
      }
  }
}
...
function reset_stats() {
  var stats = { xp: 0, passed: 0, energy: 0, selected: 1, stages: 20, diff: 1 };
...
if (stats.selected == 20 || stats.selected == 10) {
  var holes = [
    {
      object_type: "env",
      pos: [5000, 1000],
...
function get_d(a, b) {
  return Math.pow(
    Math.pow(get_pos(a)[0] - get_pos(b)[0], 2) +
      Math.pow(get_pos(a)[1] - get_pos(b)[1], 2),
    0.5
  );
}
...
function update(dt) {
  ...
  if (sp[spawned] == "t" && enemies.length + score / 100 < to_spawn) {
    ...
    enemies.push({
      ...
      name: "transport",
      object_type: "unit",
      pos: [map_width - spawned * 50, ((spawned + 1) % 5) * 300 + 5],
      sprite: new Sprite("img/enemy_1.png", [0, 0], [160, 73], 6, [0]),
    });
    ...
    enemies.push({
      ...
      object_type: "unit",
      pos: [map_width - spawned * 50, ((spawned + 1) % 5) * 300 + 5],
      sprite: new Sprite("img/enemy_2.png", [0, 0], [160, 115], 6, [0]),
    });
    spawn_vfx("teleport", enemies[enemies.length - 1].pos);
    ...
    enemies.push({
      ...
      object_type: "unit",
      pos: [map_width - spawned * 50, ((spawned + 1) % 5) * 300 + 5],
      sprite: new Sprite("img/enemy_0.png", [0, 0], [523, 300], 6, [0]),
    });
    spawn_vfx("teleport", enemies[enemies.length - 1].pos);
    ...
    enemies.push({
      ...
      object_type: "unit",
      pos: [map_width - spawned * 50, ((spawned + 1) % 5) * 300 + 5],
      sprite: new Sprite("img/enemy_4.png", [0, 0], [100, 100], 6, [0]),
    });
    spawn_vfx("teleport", enemies[enemies.length - 1].pos);
    ...
    enemies.push({
      ...
      object_type: "unit",
      pos: [7000, 1000],
      sprite: new Sprite("img/deathstar.png", [0, 0], [2000, 2000], 6, [0]),
    });
    spawn_vfx("teleport", enemies[enemies.length - 1].pos);
  }
  checkCollisions();
...
function handleInput(dt) {
  ...
  if (
    input.isDown("DOWN") ||
    (input.isDown("s") && Date.now() - lastChange > 100)
  ) {
    lastChange = Date.now();
...
function updateEntities(dt) {
  ...
  for (var i = 0; i < enemies.length; i++) {
    ...
    for (var j = 0; j < enemies.length; j++) {
      if (
        i != j &&
        Math.max(
          enemy.pos[1] - enemies[j].pos[1],
          enemy.pos[0] - enemies[j].pos[0]
        ) > 0
      ) {
        d = Math.min(
          d,
    ...
    var dg =
      180 +
      (Math.atan(
        (enemy.pos[1] + enemies[i].sprite.size[1] / 2 - player.pos[1]) /
          (enemy.pos[0] + enemies[i].sprite.size[0] / 2 - player.pos[0])
      ) *
        180) /
        Math.PI -
      180 *
        (0 -
          (enemy.pos[0] + enemies[i].sprite.size[0] / 2 - player.pos[0] < 0) *
            1);

    if (enemies[i].name == "death star" && Math.random() > 0.95) {
      enemies[i].pos[0] +=
        enemy.speed * dt * Math.cos((enemy.sprite.degrees / 180) * Math.PI);
      enemies[i].pos[1] +=
        enemy.speed * dt * Math.sin((enemy.sprite.degrees / 180) * Math.PI);
      if (dg >= enemies[i].sprite.degrees) {
        enemies[i].sprite.degrees = Math.min(
          enemies[i].sprite.degrees + enemies[i].rotation * dt,
          dg
        );
      }
      if (dg < enemies[i].sprite.degrees) {
        enemies[i].sprite.degrees = Math.max(
          enemies[i].sprite.degrees - enemies[i].rotation * dt,
          dg
        );
      }
      bullets.push({
        ...
        sprite: new Sprite(
          ...
          16,
          [0],
          enemies[i].sprite.degrees
        ),
      });
      bullets[bullets.length - 1].sprite.degrees =
        enemies[i].sprite.degrees - 180;
    }
    if (
      enemies[i].type == "fighter" &&
      (Math.abs(enemy.pos[1] + enemies[i].sprite.size[1] / 2 - player.pos[1]) <=
        600 ||
        enemies[i].name == "destroy" ||
        name == "death star") &&
      (Math.abs(enemy.pos[0] + enemies[i].sprite.size[0] / 2 - player.pos[0]) <=
        800 ||
        enemies[i].name == "destroy" ||
        name == "death star")
    ) {
      if (
        d >
        50 + Math.max(enemies[i].sprite.size[0], enemies[i].sprite.size[1])
      ) {
        if (dg >= enemies[i].sprite.degrees) {
          enemies[i].sprite.degrees = Math.min(
            enemies[i].sprite.degrees + enemies[i].rotation * dt,
    ...
    compass_markers.push({
      pos: [
        enemies[i].pos[0] * (200 / map_width) + 20,
        enemies[i].pos[1] * (125 / map_height) + 20,
      ],
      sprite: new Sprite(
        "img/enemy_marker.png",
        [0, 0],
    ...
    if (x < 0 || x > map_width || y < 0 || y > map_height) {
      ...
      enemies[i].pos[1] = Math.max(
        enemies[i].sprite.size[0] + 10,
        Math.min(enemies[i].pos[1], map_height - enemies[i].sprite.size[0] - 10)
      );
      enemies[i].sprite.degrees = 180;
      spawn_vfx("teleport", enemies[i].pos);
    }
  ...
  compass_markers.push({
    object_type: "env",
    pos: [
      player.pos[0] * (200 / map_width) + 20,
      player.pos[1] * (125 / map_height) + 20,
    ],
    sprite: new Sprite(
      "img/player_marker.png",
      [0, 0],
...
function checkCollisions() {
  ...
  for (var i = 0; i < enemies.length; i++) {
    ...
    for (var j = 0; j < bullets.length; j++) {
      ...
      if (
        bullets.length > j &&
        boxCollides(player.pos, player.sprite.size, pos2, size2) &&
        bullets[j].type == "enemy"
      ) {
        player.health--;
        bullets.splice(j, 1);
        j--;
...
function render() {
  if (!isGameOver) {
    ...
    for (var i = 0; i < stars.length * 0; i++) {
      var colors = ["#f56f42", "#00f2ff", "#fff700"];
      ...
      ctx.shadowBlur = 10;*/
      ctx.fillStyle = "red";
      ctx.arc(stars[i].pos[0] - player.pos[0], stars[i].pos[1] - player.pos[1], stars[i].size * 100, 0, 2 * Math.PI);
      stars[i].size += (Math.random() - 0.5) * 0.1;
      ctx.fill();
    }
    ctx.font = "48px serif";
    ctx.fillStyle = "red";
...
function renderEntity(entity) {
  if (isGameOver) {
    ...
    entity.sprite.url == "img/compass.png" ||
    entity.sprite.url == "img/player_marker.png" ||
    entity.sprite.url == "img/enemy_marker.png"
  ) {
    ctx.save();
    ...
    ctx.save();
    ctx.translate(
      entity.pos[0] + (800 - player.pos[0]),
      entity.pos[1] + (400 - player.pos[1])
    );
    entity.sprite.render(ctx);
    ctx.restore();
  } else {
    ctx.save();
    ctx.translate(
      entity.pos[0] + ((800 - player.pos[0]) % 2500),
      entity.pos[1] + ((400 - player.pos[1]) % 1500)
    );
    entity.sprite.render(ctx);
    ctx.restore();
  }
